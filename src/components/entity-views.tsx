...
export const EntityContainer = ({ header, search, pagination, children }: EntityContainerProps) => {
    return (
        <div className="p-4 md:px-10 md:py-6 h-full">
            <div className="mx-auto max-w-7xl w-full flex flex-col gap-y-8 h-full"> {header}
...
export const EntitySearch = ({ value, onChange, placeholder = "Search..."}: EntitySearchProps) => {
    return (
        <div className="relative ml-auto">
            ...
            <Input className="max-w-[200px] bg-background shadow-none border-border pl-8" value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}/>
...
export function EntityList<T>({ items, renderItem, emptyView, className, getKey }: EntityListProps<T>) {
    if(items.length === 0){
        return (
...
export const EntityItem = ({ href, title, subtitle, image, actions, onRemove, isRemoving, className }: EntityViewsProps) => {
                            ...
                            {onRemove && (
                                <DropdownMenu>
                                    <DropdownMenuTrigger asChild>
                                        <Button
                                        size="icon" variant="ghost" onClick={(e)=>e.stopPropagation()}>
