...
export const topologicalSort = (nodes:Node[], connections:Connection[]): Node[] => {
    ...
    for (const node of nodes) {
      ...
      try{
        ...
        if(err instanceof Error && err.message.includes("Cyclic")) {
          throw new Error("Workflow contains a cycle. Please remove cycles to execute the workflow.");
        }
        throw err;
      }
    ...
    export const sendWorkflowExecutionEvent = async (data : {workflowId: string, [key:string]:unknown}) => {
      await inngest.send({
