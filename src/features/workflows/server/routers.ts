...
export const workflowsRouter = createTRPCRouter({
    ...
    .query(async ({ ctx, input: { id } }) => {
        ...
        // Use parentheses (node) => ({ ... }) to return an object literal
        const nodes: Node[] = workflow.nodes.map((node) => ({
            id: node.id,
            type: node.type,
            // Check your schema: if it's 'position', use 'position'
    ...
    // 1. Remove the .input() line entirely if you don't need filters
